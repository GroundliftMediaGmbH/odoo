...
## Korrektur Ziel-Format bei gemeinsam installierter Groundlift Event Social App (19.0.1.2.0)

Die Event-Social-App hat auf `social.post` ein eigenes `gl_publication_kind`, das standardmäßig `story` ist, sowie `gl_publish_as_feed_post`. Die Kino-Automation überträgt ihre Wochen-/Tages-Vorgabe ab jetzt **auch** in diese Felder. Ein regulärer Kinopost zeigt in der Event-Social-Ansicht daher **Feed-Post**, und die Feed-Checkbox ist aktiviert; eine Story bleibt Story. So wird auch die Bildformatprüfung an den richtigen Ausgabekanal gekoppelt. Ist Event Social nicht installiert, werden die Felder einfach ausgelassen.

Beim manuellen Klick auf **Diese Woche jetzt erzeugen** werden außerdem bereits vorhandene, noch nicht veröffentlichte und von **dieser Kino-Automation** erzeugte Wochen- und Tagesposts auf die aktuell gespeicherten Voreinstellungen abgeglichen; ein erneuter Klick erzeugt keine Dubletten. Veröffentlichte Beiträge und Posts der Event-Automation bleiben unverändert. Normale Cron-Läufe ändern bestehende Posts nicht.

Die Menüansicht **Kino Social Automation > Social Posts** sowie die Schaltfläche **Social Posts** auf einer Kinowoche verwenden nun explizit die Kino-Ansicht, damit die Kino-Freigabe und das Kino-Veröffentlichungsformat statt der Felder der Event-Automation angezeigt werden.
