...
## Änderungen in 19.0.1.0.8

- Fix für `Freigeben & planen`: Der Button ruft keine Odoo-Social-Publish-Actions (`action_post`, `action_schedule`, `_action_post`) mehr auf.
- Stattdessen werden nur `gl_approved`, `gl_requires_approval`, `scheduled_date`, `post_method` und – falls vorhanden – der Status `scheduled` gesetzt.
- Dadurch bedeutet der Button wirklich: redaktionell freigegeben und zum geplanten Zeitpunkt vormerken, nicht sofort veröffentlichen.
