(function () {
    ...
    const app = document.getElementById("gl-kino-pos-device-setup");
    if (!app) return;

    const publicId = app.dataset.publicId || "";
    const setupToken = app.dataset.setupToken || "";
    const button = document.getElementById("gl-kino-pos-device-enroll");
    const statusEl = document.getElementById("gl-kino-pos-device-setup-status");
    ...
    async function enroll() {
        ...
        try {
            ...
            const response = await fetch("/kino-pos/device/enroll", {
                method: "POST",
            ...
            setStatus("Gerät erfolgreich gebunden. Kino POS wird geöffnet …", "success");
            window.setTimeout(() => window.location.replace(payload.redirect_url), 350);
        } catch (error) {
            console.error(error);
