...
class GlHaDeviceAccess(models.Model):
    ...
    def _compute_urls(self):
        # Prefer the host of the *current* HTTP request.  This is important on
        # Odoo.sh staging builds: a cloned database can still contain the
        # production value in ``web.base.url`` (or have it frozen), which would
        # otherwise generate a setup link pointing at the wrong database.
        # Outside an HTTP request (cron/tests/background jobs) we safely fall
        # back to Odoo's configured base URL.
        base_url = False
        try:
