...
class GLServiceShift(models.Model):
    ...
    managed_source_times = fields.Boolean(
        ...
        help=(
            'Automatisch erzeugte Schichten übernehmen die Quellzeiten. '
            'Beim Upgrade werden bestehende, noch nicht beendete Veranstaltungsschichten '
            'einmalig auf Veranstaltungsbeginn/-ende inklusive Service-Zusatzstunden neu berechnet. '
            'Sobald die Schichtzeiten danach manuell geändert werden, wird diese Kopplung beendet.'
        ),
    )
    ...
    @api.model
    def _recalculate_existing_event_shift_times(self, future_only=True):
        """Recalculate existing event-backed shifts from the event service window.

        This is intentionally mail-silent and is used by the module upgrade as well as the
        manual "Bestehende Events/Projekte einholen" action.  The shift standard is
        recalculated even for legacy shifts that were previously marked as unmanaged.

        Employee line times are moved only if they still followed the previous shift standard
        (or the raw event start/end used by older versions).  Individually changed line times
        remain untouched.  Booked employees remain booked and no availability or time-change
        mail is generated by this correction.
        """
        Shift = self.sudo()
        shifts = Shift.search([('event_id', '!=', False)])
        now = fields.Datetime.now()
        updated = 0

        for shift in shifts:
            event = shift.event_id.exists()
            if not event or not event.date_begin:
                continue

            event_end = event.date_end or event.date_begin
            if future_only and event_end and event_end < now:
                continue

            before_h, after_h = event._gl_service_get_time_offsets()
            new_start = event.date_begin - timedelta(hours=before_h)
            new_end = event_end + timedelta(hours=after_h)
            old_start, old_end = shift.start_datetime, shift.end_datetime

            # First move personnel lines that still followed the old standard.  SQL is not
            # used here so custom extensions still see normal writes, while the context below
            # suppresses any time-change mail caused solely by this migration correction.
            for line in shift.line_ids:
                line_vals = {}
                if line.planned_start_datetime in (False, old_start, event.date_begin):
                    if line.planned_start_datetime != new_start:
                        line_vals['planned_start_datetime'] = new_start
                if line.planned_end_datetime in (False, old_end, event_end):
                    if line.planned_end_datetime != new_end:
                        line_vals['planned_end_datetime'] = new_end
                if line_vals:
                    line.with_context(
    ...
    def action_import_sources(self):
        ...
        if 'date_begin' in Event._fields:
            ...
            for event in relevant_events:
                shift = self._sync_from_event(event)
                if shift:
                    count += 1
            # Existing legacy event shifts may still have managed_source_times=False.
            # Recalculate their current/future service windows explicitly and without mails.
            self._recalculate_existing_event_shift_times(future_only=True)
        return {
